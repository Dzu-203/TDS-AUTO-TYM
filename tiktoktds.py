...
def login_tds(token):
    try:
        r = requests.get('https://traodoisub.com/api/?fields=profile&access_token=' +
                         token, headers=headers, timeout=5).json()
        if 'success' in r:
            os.system('cls' if os.name == 'nt' else 'clear')
            print(
                Colors.green + f"Đăng nhập thành công!\nUser: {Colors.yellow + r['data']['user'] + Colors.green} | Xu hiện tại: {Colors.yellow + r['data']['xu']}")
            return 'success'
        else:
...
def load_job(type_job, token):
    try:
        r = requests.get('https://traodoisub.com/api/?fields='+type_job +
                         '&access_token='+token, headers=headers, timeout=5).json()
        if 'data' in r:
            return r
        elif "countdown" in r:
...
def duyet_job(type_job, token, uid):
    try:
        r = requests.get(
            f'https://traodoisub.com/api/coin/?type={type_job}&id={uid}&access_token={token}', headers=headers, timeout=5).json()
        if "cache" in r:
            return r['cache']
        elif "success" in r:
            dai = f'{Colors.yellow}------------------------------------------'
            print(dai)
            print(
                f"{Colors.cyan}Nhận thành công {r['data']['job_success']} nhiệm vụ | {Colors.green}{r['data']['msg']} | {Colors.yellow}{r['data']['xu']}")
            print(dai)
            return 'error'
        else:
...
def check_tiktok(id_tiktok, token):
    try:
        r = requests.get('https://traodoisub.com/api/?fields=tiktok_run&id=' +
                         id_tiktok+'&access_token='+token, headers=headers, timeout=5).json()
        if 'success' in r:
            os.system('cls' if os.name == 'nt' else 'clear')
            print(
                Colors.green + f"{r['data']['msg']}|ID: {Colors.yellow + r['data']['id'] + Colors.green}")
            return 'success'
        else:
            print(Colors.red + f"{r['error']}\n")
            return 'error_token'
...
{Colors.yellow}{gach}
'''
print(Colors.red + Center.XCenter(Box.DoubleCube("Tool TDS tiktok")))
...
while True:
    try:
        ...
        token_tds = Write.Input("Nhập token TDS:", Colors.green)
        cache = 'new'
            ...
            while True:
                ...
                try:
                    choice = int(Write.Input(
                        "Lựa chọn của bạn là (Ví dụ: sử dụng acc cũ nhập 1):", Colors.green, interval=0.0025))
                    if choice in [1, 2]:
                        break
                    else:
                        os.system('cls' if os.name == 'nt' else 'clear')
...
if check_log == 'success':
    # Nhập user tiktok
    while True:
        id_tiktok = Write.Input(
            "Nhập ID tiktok chạy (lấy ở mục cấu hình web):", Colors.green, interval=0.0025)
        for _ in range(3):
    ...
    while True:
        ...
        try:
            choice = int(Write.Input(
                "Lựa chọn nhiệm vụ muốn làm (Ví dụ: Follow nhập 1):", Colors.green, interval=0.0025))
            if choice in [1, 2]:
                break
            else:
                os.system('cls' if os.name == 'nt' else 'clear')
    ...
    while True:
        try:
            delay = int(Write.Input(
                "Thời gian delay giữa các job (giây):", Colors.green, interval=0.0025))
            if delay > 1:
                break
            else:
                os.system('cls' if os.name == 'nt' else 'clear')
    ...
    while True:
        try:
            max_job = int(Write.Input(
                "Dừng lại khi làm được số nhiệm vụ là:", Colors.green, interval=0.0025))
            if max_job > 9:
                break
            else:
                os.system('cls' if os.name == 'nt' else 'clear')
    ...
    upload = driver.find_element(
        By.XPATH, "//body/div[@id='app']/div[1]/div[1]/div[3]/div[1]/a[1]/div[1]")
    dem_tong = 0
    while True:
        ...
        if isinstance(list_job, dict) == True:
            for job in list_job['data']:
                ...
                if check_duyet != 'error':
                    ...
                    print(
						f'{Colors.yellow}[{dem_tong}] {Colors.red}| {Colors.cyan}{t_now} {Colors.red}| {Colors.pink}{type_type} {Colors.red}| {Colors.light_gray}{uid}')
                    if check_duyet > 9:
                        sleep(3)
                    ...
                    if dem_tong == max_job:
                        ...
                        for i in range(delay, -1, -1):
                            print(Colors.green + 'Vui lòng đợi: ' +
                                  str(i)+' giây', end=('\r'))
                            sleep(1)
                            if dem_tong == max_job:
